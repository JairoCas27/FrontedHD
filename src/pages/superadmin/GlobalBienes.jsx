...
export default function GlobalBienes() {
  ...
  const allOccupants = React.useMemo(() => {
    ...
    apartments.forEach(apt => {
      if (apt.nombrePropietario) names.push({ label: `${apt.nombrePropietario} (Due�o, Apt ${apt.numero})`, nombre: apt.nombrePropietario, dni: '' })
  ...
  const handleRegisterEntry = async (e) => {
    e.preventDefault()
    if (!entryForm.placa) return showToast('Selecciona un veh�culo', 'error')
    setSaving(true)
    try {
  ...
  const handleEditVehicle = async (e) => {
    e.preventDefault()
    if (!vehicleForm.marca || !vehicleForm.placa) return showToast('Completa marca y placa', 'error')
    if (!vehicleForm.id) return showToast('Error: veh�culo no identificado', 'error')
    setSaving(true)
    try {
      const payload = {
        marca: vehicleForm.marca, color: vehicleForm.color, modelo: vehicleForm.modelo,
  ...
  const handleUnassignVehicle = async (vehicle) => {
    try {
      ...
      showToast('Veh�culo retirado del estacionamiento')
      loadData(condoId)
    } catch (e) { showToast('Error: ' + e.message, 'error') }
  }
  ...
  const handleCartLoan = async (e) => {
    e.preventDefault()
    if (!cartLoanForm.codigoCarrito) return showToast('No hay carritos disponibles con c�digo', 'error')
    setSaving(true)
    try {
  ...
  const handleAssignVehicleToSpot = async (e) => {
    ...
    try {
      ...
      showToast('Veh�culo asignado al estacionamiento')
      setShowModal(null)
  ...
  return (
    <div style={styles.container} className="gb-container">
      ...
      <EncabezadoTabla titulo="Bienes Comunes" subtitulo="Gesti�n integral de estacionamientos, veh�culos y carritos" />
          ...
          <div style={{
            ...
            {filteredCondominios.length === 0 && (
              <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>
                <p style={{ fontWeight: 600, margin: 0 }}>Ning�n condominio coincide con tu b�squeda</p>
              </div>
            )}
          </div>
        </div>
        ...
        <>
          <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1rem", flexWrap: "wrap" }}>
            {[
              ...
              { key: 'gestion', label: 'Gesti�n', icon: <FiSettings size={14} /> },
              { key: 'reservas', label: 'Reservas', icon: <FiClock size={14} /> },
              { key: 'vehiculos', label: 'Veh�culos', icon: <FiNavigation2 size={14} /> },
          ...
          {activeTab === 'dashboard' && (
            <>
              <div style={{ display: "grid", gap: "1rem", marginBottom: "1.5rem" }} className="gb-stats-grid">
                ...
                <StatCard icon={<FiUsers />} label="Capacidad Total" value={stats.capacidadTotal} sub={`${stats.ocupacionActual} veh�culos ahora (${stats.capacidadTotal > 0 ? ((stats.ocupacionActual / stats.capacidadTotal) * 100).toFixed(0) : 0}%)`} color="#3b82f6" />
                ...
                <StatCard icon={<FiNavigation2 />} label="Veh�culos Registrados" value={stats.totalVehicles} color="#f59e0b" />
                <StatCard icon={<FiLogIn />} label="Veh�culos Dentro" value={stats.activeEntries.length} sub={`${stats.disponibleParking} spots libres`} color="#10b981" />
              </div>

              <div style={{ display: "grid" }} className="gb-grid-2">
                <div style={styles.card}>
                  <div style={styles.cardHeader}>
                    <span style={{ fontWeight: 800, fontSize: "0.8rem", color: "#0f172a" }}>Ocupaci�n de Estacionamientos</span>
                          ...
                          content={({ active, payload }) => {
                            ...
                            return (
                              <div style={{ backgroundColor: "#fff", border: "1px solid #e2e8f0", borderRadius: "0.5rem", padding: "0.5rem 0.75rem", boxShadow: "0 4px 12px rgba(0,0,0,0.08)", fontSize: "0.75rem" }}>
                                ...
                                <div style={{ color: "#475569" }}>Capacidad: {d.capacidad} veh�culos</div>
                ...
                <div style={styles.card}>
                  <div style={styles.cardHeader}>
                    <span style={{ fontWeight: 800, fontSize: "0.8rem", color: "#0f172a" }}>Veh�culos dentro ahora</span>
                  </div>
                  <div style={{ maxHeight: "200px", overflowY: "auto" }}>
                    {stats.activeEntries.length === 0 ? (
                      <div style={{ padding: "1.5rem", textAlign: "center", color: "#94a3b8", fontSize: "0.8rem" }}>Ning�n veh�culo dentro</div>
                    ...
                    {stats.activeEntries.length > 10 && <div style={{ padding: "0.5rem", textAlign: "center", fontSize: "0.7rem", color: "#94a3b8" }}>+{stats.activeEntries.length - 10} m�s</div>}
                  </div>
                </div>
              </div>
                ...
                <div className="gb-grid-3" style={{ display: "grid", gap: "1rem", marginTop: "1.5rem" }}>
                  <div style={styles.card}>
                    <div style={{ padding: "1.25rem", textAlign: "center" }}>
                      <div style={{ fontSize: "0.7rem", fontWeight: 700, color: "#94a3b8", textTransform: "uppercase" }}>Veh�culos dentro</div>
                          ...
                          <div key={p.id} style={{
                            ...
                            <div style={{ padding: "0.75rem" }}>
                              ...
                              <div style={{ fontSize: "0.65rem", color: "#94a3b8", marginBottom: "0.3rem" }}>
                                {p.cantidadActual ?? 0}/{p.capacidadMaxima || '8'} veh�culos
                              </div>
                            ...
                            {vehiclesInSpot.length > 0 && (
                              <div style={{ borderTop: "1px solid #e2e8f0", backgroundColor: "#f8fafc", padding: "0.5rem 0.75rem" }}>
                                <div style={{ fontSize: "0.6rem", fontWeight: 700, color: "#64748b", marginBottom: "0.3rem" }}>Veh�culos:</div>
          ...
          {activeTab === 'gestion' && (
            <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                  ...
                  {entryOpen && (
                    <form onSubmit={handleRegisterEntry} style={{ padding: "1.25rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                      ...
                      {entryFilters.aptId && (() => {
                        ...
                        if (!apt) return null
                        const occupants = []
                        if (apt.nombrePropietario) occupants.push({ label: `${apt.nombrePropietario} (Due�o)`, nombre: apt.nombrePropietario, tipo: 'PROPIETARIO', dni: '' })
                      ...
                      <label style={{ ...styles.label, fontSize: "0.7rem", color: "#475569", textAlign: "left" }}>Seleccionar veh�culo</label>
                      ...
                      <div className="gb-form-grid" style={{ display: "grid", gap: "0.75rem" }}>
                        <div>
                          <label style={{ ...styles.label, fontSize: "0.7rem", color: "#475569", textAlign: "left" }}>M�todo</label>
                      ...
                      {vehicles.length === 0 && <div style={{ fontSize: "0.75rem", color: "#ef4444" }}>No hay veh�culos registrados en el sistema</div>}
                ...
                <div style={styles.card}>
                  ...
                  {exitOpen && (
                    <form onSubmit={handleRegisterExit} style={{ padding: "1.25rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                      <label style={{ ...styles.label, fontSize: "0.7rem", color: "#475569", textAlign: "left" }}>Seleccionar veh�culo dentro</label>
                      <DataList value={exitLogText} onChange={e => { setExitLogText(e.target.value); const s = stats.activeEntries.find(l => `${l.placa} � ${l.ocupante} � Entrada: ${formatDate(l.fechaEntrada)}` === e.target.value); if (s) setExitForm(f => ({ ...f, idLogAcceso: String(s.id) })) }} required style={styles.select}>
                        <option value="">Seleccionar veh�culo dentro</option>
                        ...
                        return sel ? (
                          <div style={{ fontSize: "0.75rem", backgroundColor: "#fef2f2", padding: "0.75rem", borderRadius: "0.5rem", color: "#991b1b" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.25rem" }}>
                              <span><strong>Veh�culo:</strong> {sel.placa}</span>
                              <span><strong>M�todo:</strong> {sel.metodo}</span>
                            </div>
                      ...
                      {stats.activeEntries.length === 0 && <div style={{ fontSize: "0.75rem", color: "#94a3b8" }}>No hay veh�culos dentro</div>}
              ...
              <div style={styles.card}>
                <div style={styles.cardHeader}>
                  <span style={{ fontWeight: 800, fontSize: "0.85rem", color: "#0f172a", display: "flex", alignItems: "center", gap: "0.4rem" }}>
                    <FiCalendar size={14} color={colorSuper} /> Bit�cora de Accesos Vehiculares
                  </span>
                  <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                    <div style={{ position: "relative" }}>
                      <FiSearch size={13} style={{ position: "absolute", left: "0.5rem", top: "50%", transform: "translateY(-50%)", color: "#94a3b8" }} />
                      <input type="text" placeholder="Buscar placa, veh�culo..." value={logSearch} onChange={e => { setLogSearch(e.target.value); setLogPage(1) }}
                      ...
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.82rem" }}>
                        <thead>
                          <tr style={{ backgroundColor: "#f8fafc", color: "#64748b", fontWeight: 700, fontSize: "0.65rem", textTransform: "uppercase", borderBottom: "1px solid #e2e8f0" }}>
                            ...
                            <th style={{ padding: "0.75rem" }}>Veh�culo</th>
                            ...
                            <th style={{ padding: "0.75rem" }}>M�todo</th>
              ...
              <div style={styles.card}>
                ...
                {reserveOpen && (
                  <form onSubmit={async (e) => {
                    e.preventDefault()
                    if (!reserveForm.placa) return showToast('Selecciona un veh�culo', 'error')
                    ...
                    <div style={{ fontSize: "0.7rem", color: "#64748b" }}>Asigna un estacionamiento a un veh�culo por tiempo limitado. Se registrar� la entrada y se liberar� autom�ticamente al vencer.</div>
                    <label style={{ ...styles.label, fontSize: "0.7rem", color: "#475569", textAlign: "left" }}>Seleccionar torre</label>
                      ...
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.82rem" }}>
                        <thead>
                          <tr style={{ backgroundColor: "#f8fafc", color: "#64748b", fontWeight: 700, fontSize: "0.65rem", textTransform: "uppercase", borderBottom: "1px solid #e2e8f0" }}>
                            ...
                            <th style={{ padding: "0.75rem" }}>Estacionamiento</th>
                            <th style={{ padding: "0.75rem" }}>Ocupante</th>
                            <th style={{ padding: "0.75rem" }}>Duraci�n</th>
                            <th style={{ padding: "0.75rem" }}>Entrada</th>
            ...
            <div style={styles.card}>
              <div style={styles.cardHeader}>
                <span style={{ fontWeight: 800, fontSize: "0.9rem", color: "#0f172a", display: "flex", alignItems: "center", gap: "0.4rem" }}>
                  <FiNavigation2 size={15} color={colorSuper} /> Veh�culos Registrados ({vehicles.length})
                </span>
                <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                  <div style={{ position: "relative" }}>
                    <FiSearch size={13} style={{ position: "absolute", left: "0.5rem", top: "50%", transform: "translateY(-50%)", color: "#94a3b8" }} />
              ...
              {(() => {
                ...
                if (filtered.length === 0) return (
                  <div style={{ padding: "2rem", textAlign: "center", color: "#94a3b8", fontSize: "0.85rem" }}>No hay veh�culos registrados</div>
                )
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.82rem" }}>
                      <thead>
                        <tr style={{ backgroundColor: "#f8fafc", color: "#64748b", fontWeight: 700, fontSize: "0.65rem", textTransform: "uppercase", borderBottom: "1px solid #e2e8f0" }}>
                          ...
                          <th style={{ padding: "0.75rem" }}>Estacionamiento</th>
                          <th style={{ padding: "0.75rem 1rem", textAlign: "right" }}>Acci�n</th>
                        </tr>
                      </thead>
                      <tbody style={{ color: "#334155" }}>
                        {paged.map(v => {
          ...
          {activeTab === 'carritos' && (<>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              <div style={styles.card}>
                <div style={{ ...styles.cardHeader, cursor: "pointer" }} onClick={() => setCartOpen(!cartOpen)}>
                  <span style={{ fontWeight: 800, fontSize: "0.85rem", color: "#0f172a" }}>Pr�stamo de Carrito</span>
                ...
                {cartOpen && (
                  <form onSubmit={handleCartLoan} style={{ padding: "1.25rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                    ...
                    {cartFilters.aptId && (() => {
                      ...
                      if (!apt) return null
                      const occupants = []
                      if (apt.nombrePropietario) occupants.push({ label: `${apt.nombrePropietario} (Due�o)`, nombre: apt.nombrePropietario, tipo: 'PROPIETARIO', id: apt.idPropietario, dni: '' })
              ...
              {activeLoans.length > 0 && (
                <div style={styles.card}>
                  <div style={styles.cardHeader}>
                    <span style={{ fontWeight: 800, fontSize: "0.85rem", color: "#0f172a" }}>Pr�stamos Activos ({activeLoans.length})</span>
                  </div>
                  <div style={{ maxHeight: "200px", overflowY: "auto" }}>
            ...
            {allCartLoans.length > 0 && (
              <div style={{ marginTop: "1.5rem" }}>
                <div style={styles.card}>
                  <div style={styles.cardHeader}>
                    <span style={{ fontWeight: 800, fontSize: "0.85rem", color: "#0f172a" }}>Historial de Pr�stamos ({allCartLoans.length})</span>
                    <div style={{ position: "relative" }}>
                      <FiSearch size={13} style={{ position: "absolute", left: "0.5rem", top: "50%", transform: "translateY(-50%)", color: "#94a3b8" }} />
                      <input type="text" placeholder="Buscar pr�stamo..." value={loanSearch} onChange={e => { setLoanSearch(e.target.value); setLoanPage(1) }}
                        style={{ ...styles.input, padding: "0.35rem 0.5rem 0.35rem 1.6rem", fontSize: "0.75rem", width: "180px" }} />
                    </div>
                  </div>
                  {(() => {
                        ...
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.72rem" }}>
                          <thead>
                            <tr style={{ backgroundColor: "#f8fafc", borderBottom: "2px solid #e2e8f0" }}>
                              ...
                              <th style={thStyle}>Pr�stamo</th>
                              <th style={thStyle}>Devoluci�n</th>
                            ...
                            {paged.map((loan, i) => {
                              ...
                              return (
                                <tr key={loan.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                                  ...
                                  <td style={tdStyle}>{loan.solicitante === 'PROPIETARIO' ? <span style={styles.badge("#e0e7ff", "#4338ca")}>Due�o</span> : <span style={styles.badge("#fef3c7", "#d97706")}>Inquilino</span>}</td>
        ...
        <div style={styles.modalOverlay} onClick={() => setCartTicket(null)}>
          <div style={styles.modalBox} className="gb-modal-box" onClick={e => e.stopPropagation()}>
            <div style={{ padding: "1.5rem", textAlign: "center" }}>
              ...
              <h3 style={{ margin: "0 0 0.25rem", fontSize: "1.1rem", fontWeight: 800, color: "#1e293b" }}>Comprobante de Pr�stamo</h3>
              <div style={{ fontSize: "0.75rem", color: "#94a3b8" }}>Pr�stamo de Carrito � #{cartTicket.id}</div>
            </div>
            <div style={{ padding: "0 1.5rem 1.5rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              <div className="gb-form-grid" style={{ display: "grid", gap: "0.75rem" }}>
                ...
                <div>
                  <div style={styles.label}>Fecha de Pr�stamo</div>
            ...
            <form onSubmit={handleCreate}>
              <div style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
                {showModal === 'createCart' ? (
                  <div>
                    <label style={styles.label}>C�digo del carrito</label>
                  ...
                  <>
                    ...
                    <div>
                      <label style={styles.label}>Tipo de veh�culo</label>
                    ...
                    <div>
                      <label style={styles.label}>Capacidad m�xima (celdas)</label>
      ...
      {showModal === 'vehicle' && (
        <div style={styles.modalOverlay} onClick={() => { setShowModal(null); setVehicleForm({ id: null, marca: '', color: 'BLANCO', modelo: '', placa: '', tipo: 'AUTO', inquilinoId: '' }); setVehInquilinoText('') }}>
          <div style={styles.modalBox} className="gb-modal-box" onClick={e => e.stopPropagation()}>
            <div style={{ padding: "1.25rem 1.5rem", borderBottom: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h3 style={{ margin: 0, fontSize: "1rem", fontWeight: 800, color: "#1e293b" }}>
                <FiNavigation2 size={16} style={{ marginRight: 8, verticalAlign: "middle" }} />{vehicleForm.id ? 'Editar Veh�culo' : 'Nuevo Veh�culo'}
              </h3>
          ...
          <div style={styles.modalBox} className="gb-modal-box" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleUpdateParkingConfig}>
              <div style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
                ...
                <div>
                  <label style={styles.label}>Capacidad m�xima (veh�culos)</label>
                ...
                <div style={{ fontSize: "0.75rem", color: "#64748b", backgroundColor: "#f8fafc", padding: "0.75rem", borderRadius: "0.5rem" }}>
                  {configForm.tipoVehiculo === 'AUTO' ? 'Matriz 4�4: los autos se muestran en una cuadr�cula de 4 columnas.' : 'Matriz 4�2: las motos se muestran en una cuadr�cula de 4 columnas y 2 filas.'}
                </div>
              </div>
              <div style={{ padding: "1rem 1.5rem", borderTop: "1px solid #f1f5f9", display: "flex", justifyContent: "flex-end", gap: "0.75rem", backgroundColor: "#f8fafc" }}>
                <button type="submit" disabled={saving} style={{ backgroundColor: colorSuper, border: "none", color: "#fff", padding: "0.5rem 1.25rem", borderRadius: "0.5rem", fontSize: "0.85rem", fontWeight: 600, cursor: "pointer", opacity: saving ? 0.6 : 1 }}>
      ...
      {showModal === 'pickVehicle' && (
        <div style={styles.modalOverlay} onClick={() => { setShowModal(null); setAssignVehicleForm({ idEstacionamiento: '', idVehiculo: '' }); setPickVehicleText('') }}>
          <div style={{ ...styles.modalBox, maxWidth: "420px" }} className="gb-modal-box" onClick={e => e.stopPropagation()}>
            <div style={{ padding: "1.25rem 1.5rem", borderBottom: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h3 style={{ margin: 0, fontSize: "1rem", fontWeight: 800, color: "#1e293b" }}>Asignar Veh�culo</h3>
            ...
            <form onSubmit={handleAssignVehicleToSpot} style={{ padding: "1.25rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              ...
              <div>
                <label style={styles.label}>Veh�culo</label>
              ...
              {vehicles.filter(v => !v.idEstacionamiento).length === 0 && (
                <div style={{ fontSize: "0.75rem", color: "#ef4444" }}>No hay veh�culos disponibles</div>
              )}
            ...
            <div style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              {detailItem.tipoVehiculo ? (
                <>
                  ...
                  <Row label="Tipo de veh�culo">{detailItem.tipoVehiculo || 'MIXTO'}</Row>
                  <Row label="Capacidad m�xima">{detailItem.capacidadMaxima ?? '8'}</Row>
                  <Row label="Ocupaci�n actual">{detailItem.cantidadActual ?? 0}</Row>
                  ...
                  {(() => {
                    ...
                    return spotVehicles.length > 0 ? (
                      <div style={{ borderTop: "1px solid #f1f5f9", paddingTop: "0.75rem" }}>
                        <div style={{ fontSize: "0.75rem", fontWeight: 600, color: "#64748b", marginBottom: "0.5rem" }}>Veh�culos ocupando:</div>
                        {spotVehicles.map(v => (
                ...
                <>
                  <Row label="C�digo">{detailItem.codigo || `#${detailItem.id}`}</Row>
                  <Row label="Estado">
                    <span style={{
                      padding: "0.15rem 0.5rem", borderRadius: "999px", fontSize: "0.7rem", fontWeight: 700,
            ...
            <div id="ticket-content" style={{ padding: "1.5rem", textAlign: "center", fontFamily: "'Courier New', monospace" }}>
              ...
              <div style={{ fontSize: "0.8rem", textAlign: "left", margin: "0.5rem 0" }}>
                {[
                  ...
                  { l: 'Veh�culo', v: `${ticket.vehiculoMarca} ${ticket.vehiculoModelo}`.trim() },
        ...
        <div style={styles.modalOverlay} onClick={() => setConfirmDelete(null)}>
          <div style={{ ...styles.modalBox, maxWidth: "400px" }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: "1.5rem", textAlign: "center" }}>
              ...
              <h3 style={{ margin: "0 0 0.5rem", fontSize: "1rem", fontWeight: 800, color: "#1e293b" }}>Confirmar eliminaci�n</h3>
